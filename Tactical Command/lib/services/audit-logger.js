...
class AuditLogger {
    ...
    async sign(data) {
        ...
        const hmac = (0, crypto_1.createHmac)('sha256', this.signingKey);
     ...
     */
    getAgentActivity(agentId, timeWindow) {
        let agentLogs = this.logs.filter(log => log.agent_id === agentId);
        if (timeWindow) {
            agentLogs = agentLogs.filter(log => log.timestamp >= timeWindow.start && log.timestamp <= timeWindow.end);
        }
        const eventsByType = {};
    ...
    createSummaryReport(timeWindow) {
        let logs = [...this.logs];
        if (timeWindow) {
            logs = logs.filter(log => log.timestamp >= timeWindow.start && log.timestamp <= timeWindow.end);
        }
        const eventsByType = {};
        const agentActivity = {};
        let policyViolations = 0;
