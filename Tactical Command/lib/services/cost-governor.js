...
class CostGovernor {
    ...
    estimateRequest(envelope) {
        ...
        const estimatedCost = (estimatedInputTokens * this.pricing.gpt4.inputTokens) +
     ...
     */
    getRecentRequests(agentId, windowSeconds) {
    ...
    getCostBreakdown(agentId) {
        ...
        return {
            ...
            byAgent: Object.fromEntries(Object.entries(usage).map(([id, u]) => [id, u.cost]))
        };
    }
    /**
     * Check if agent is approaching limits
     */
    checkLimitWarnings(agentId) {
        const usage = this.getAgentUsage(agentId);
        const warnings = [];
