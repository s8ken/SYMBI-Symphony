import { AgentManifest, AGENT_ROLES } from '../types/agent-types'
...
export const SYMBI_AGENT_SPECS: Record<string, AgentManifest> = {
 ...
 */
export function getSymbiAgentSpec(agentId: string): AgentManifest | undefined {
  return SYMBI_AGENT_SPECS[agentId]
}

/**
 * Get all SYMBI agent specifications
 */
export function getAllSymbiAgentSpecs(): AgentManifest[] {
  return Object.values(SYMBI_AGENT_SPECS)
}

/**
 * Validate agent against SYMBI specifications
 */
export function validateSymbiAgent(agentId: string): boolean {
  return agentId in SYMBI_AGENT_SPECS
}

/**
 * Get agents by clearance level
 */
export function getAgentsByClearance(clearance: string): AgentManifest[] {
  return Object.values(SYMBI_AGENT_SPECS).filter(agent => agent.clearance === clearance)
}

/**
 * Get agents by compartment
 */
export function getAgentsByCompartment(compartment: string): AgentManifest[] {
  return Object.values(SYMBI_AGENT_SPECS).filter(agent => 
    agent.compartments?.includes(compartment)
  )
}
 ...
 */
export async function initializeSymbiAgents(messageBus: any): Promise<void> {
